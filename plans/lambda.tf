...
resource "aws_cloudwatch_event_target" "schedule_lambda" {
  count     = var.app_env == "Prod" ? 1 : 0
  rule      = one(aws_cloudwatch_event_rule.feed_processor_charles_haleys_schedule[*].name)
...
resource "aws_cloudwatch_log_group" "charles_haleys_logs" {
  skip_destroy      = var.app_env == "Prod"
  name              = "/aws/lambda/${aws_lambda_function.feed_processor_charles_haleys.function_name}"
